...
x = np.ones(1, dtype=np.int8)
y = x
z = y
for zt in range(tt.shape[0]):
    for yt in range(tt.shape[1]):
        for xt in range(tt.shape[2]):
            if tt[zt, yt, xt] == True:
                x = np.append(x, xt)
                y = np.append(y, yt)
                z = np.append(z, -zt)

np.savetxt('./scene.txt', (x,y,z))
pcd =open3d.io.read_point_cloud('scene.txt', format='xyz')
# # print(pcd)
open3d.visualization.draw_geometries([pcd], width=1200, height=600)